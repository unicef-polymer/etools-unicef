...
export function typedKeysFactory<Keys extends Record<string, unknown>>() {
  return {
    get<T extends ObjectLookupString<Keys>>(
      key: T,
    ...
    translate<T extends ObjectLookupString<Keys>>(
      key: T,
      values?: Values | ValuesCallback | null,
    ...
    translateUnsafeHTML<T extends ObjectLookupString<Keys>>(
      key: T,
      values?: Values | ValuesCallback | null,
      config?: ITranslateConfig
    ) {
...
export type ObjectLookupString<
  T,
  MaxDepth extends number = 10,
  Separator extends string = '.'
> = ObjectLookupStringHelper<T, ``, MaxDepth, 0, Separator>;
