 ...
 * @param config
 */
export function interpolate(text: string, values: Values | ValuesCallback | null, _config: ITranslateConfig): string {
  return Object.entries(extract(values || {})).reduce(
    (text, [key, value]) => text.replace(new RegExp(`{{[  ]*${key}[  ]*}}`, `gm`), String(extract(value))),
    text
  );
}
